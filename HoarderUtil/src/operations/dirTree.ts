...
const dirTree = async (options: FsOpFlags) => {
  ...
  if (dirListStr?.length === 0)
    return msgShortcuts.errorAndQuit('No valid dir paths provided');
  ...
  if (input === 'n')
    return msgShortcuts.messageAndQuit('Canceling on user request. Sayonara.');

  const created = await withTimer(
    () =>
      walk(absPath, dirListStr, ({ exists, fullPath }) =>
        exists
          ? output.log(`Skipped creating ${fullPath} - existing`)
          : fs.mkdir(fullPath)
      ),
